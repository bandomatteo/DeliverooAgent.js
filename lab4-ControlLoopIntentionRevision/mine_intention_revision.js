...
//     }
//     optionsGeneration();
//     await new Promise( res => setTimeout( res ) ); // very important in order not to block the code 
// }
...
class IntentionRevisionReplace extends IntentionRevision {
  async push(predicate) {
    ...
    if (last) {
      last.stop(); // this is an async stop, so we need to build something in order to cope with this one 
    }
  }
}
...
class BlindMove extends Plan {
  ...
  async execute(go_to, x, y) {
    while (me.x != x || me.y != y) {
      ...
      if (x > me.x) moved_horizontally = await client.emitMove("right");
      // status_x = await this.subIntention( 'go_to', {x: me.x+1, y: me.y} );
      else if (x < me.x) moved_horizontally = await client.emitMove("left");
      // status_x = await this.subIntention( 'go_to', {x: me.x-1, y: me.y} );
      ...
      if (y > me.y) moved_vertically = await client.emitMove("up");
      // status_x = await this.subIntention( 'go_to', {x: me.x, y: me.y+1} );
      else if (y < me.y) moved_vertically = await client.emitMove("down");
      // status_x = await this.subIntention( 'go_to', {x: me.x, y: me.y-1} );
